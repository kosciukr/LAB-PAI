...
class KsiazkiController extends AbstractActionController
{
    ...
    {
        $id = (int)$this->params()->fromRoute('id');//get route parameter
        if (empty($id)) {
            $this->redirect()->toRoute('ksiazki');
        }
        foreach ($this->ksiazkaForm->getElements() as $elem) {
            $elem->setAttributes(['readonly' => 'readonly', 'disabled' => true]);
        }
        $this->ksiazkaForm->get('zapisz')->setValue('Wróć')->setAttribute('disabled', false);//zapisz na wróć
        $request = $this->getRequest();
        if ($request->isPost()) {
            return $this->redirect()->toRoute('ksiazki');
        } else {
            $ksiazkaInfo = $this->ksiazka->pobierz($id);
            $this->ksiazkaForm->setData($ksiazkaInfo);
        }
        $viewModel = new ViewModel(['tytul' => 'Szczegóły wybranej książki', 'form' => $this->ksiazkaForm]);
    ...
    public function usunAction()
    {
        ...
        $this->ksiazkaForm->get('zapisz')->setValue('Usuń')->setAttribute('disabled', false);
        $request = $this->getRequest();
        if ($request->isPost()) {
        ...
        $viewModel = new ViewModel(['tytul' => 'Czy chcesz usunąć wybraną książkę?', 'form' => $this->ksiazkaForm]);
        $viewModel->setTemplate('application/ksiazki/dodaj');
